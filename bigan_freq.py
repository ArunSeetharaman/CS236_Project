...
def generate_keras_input(mode = 'train'):
    ...
    while(1):
        for file in files:
            ...
            x = np.stack((np.real(kspace), np.imag(kspace)), -1)
...
class GAN(object):
    ...
    def discriminator(self):
        ...
        inp = Input(shape = [im_size, im_size, 2])
    ...
    def generator(self):
        ...
        x = Conv2D(filters = 2, kernel_size = 1, padding = 'same', kernel_initializer = 'he_normal')(x)
    ...
    def AdModel(self):
        ...
        ri = Input(shape = [im_size, im_size, 2])
        
        ui = Input(shape = [im_size, im_size, 2])
...
class BiGAN(object):
    ...
    def evaluate(self, num = 0):
        ...
        generated_images = generated_images[:,:,:,0] + 1j*generated_images[:,:,:,1]
        generated_images = np.fft.ifftshift(generated_images, (1,2))
        generated_images = np.fft.ifft2(generated_images)
        generated_images = np.absolute(generated_images)
        generated_images = 2*(generated_images-np.min(generated_images))/(np.max(generated_images)-np.min(generated_images))-1
        
        data = next(self.test_data)
        real_images = data[0]
        real_images = real_images[:,:,:,0] + 1j*real_images[:,:,:,1]
        real_images = np.fft.fftshift(real_images, (1,2))
        real_images = np.fft.ifft2(real_images)
        real_images = np.absolute(real_images)
        real_images = 2*(real_images-np.min(real_images))/(np.max(real_images)-np.min(real_images))-1
        ...
        reconstructed_images = reconstructed_images[:,:,:,0] + 1j*reconstructed_images[:,:,:,1]
        reconstructed_images = np.fft.ifftshift(reconstructed_images, (1,2))
        reconstructed_images = np.fft.ifft2(reconstructed_images)
        reconstructed_images = np.absolute(reconstructed_images)
        reconstructed_images = 2*(reconstructed_images-np.min(reconstructed_images))/(np.max(reconstructed_images)-np.min(reconstructed_images))-1
        ...
        plt.savefig("BiGAN_freq/Results/i"+str(num)+".png")
        ...
        plt.imshow(c1, cmap='gray')
        plt.savefig("BiGAN_freq/Results/i"+str(num)+"-ema.png")
    ...
    def saveModel(self, model, name, num):
        json = model.to_json()
        with open("BiGAN_freq/Models/"+name+".json", "w") as json_file:
            json_file.write(json)

        model.save_weights("BiGAN_freq/Models/"+name+"_"+str(num)+".h5")

    def loadModel(self, name, num):

        file = open("BiGAN_freq/Models/"+name+".json", 'r')
        json = file.read()
        file.close()

        mod = model_from_json(json)
        mod.load_weights("BiGAN_freq/Models/"+name+"_"+str(num)+".h5")
