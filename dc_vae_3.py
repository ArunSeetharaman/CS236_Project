...
class VariationalAutoencoder():
    def __init__(self):
        ...
        self.variational_autoencoder = Model(under, [reconstructed_img, params])
        
        self.variational_autoencoder.compile(loss=['mse', dkl_normal],
    ...
    def build_decoder(self):
        ...
        model.add(BatchNormalization(momentum=0.8))
        model.add(LeakyReLU(alpha=0.2))
        model.add(Dense(64 * 40 * 40))
        model.add(LeakyReLU(alpha=0.2))
        model.add(BatchNormalization(momentum=0.8))
        model.add(Reshape((40, 40, 64)))
    ...
    def generate_keras_input(self, mode = 'train'):
        ...
        while(1):
            for file in files:
                ...
                yield (under, [x, params])

def dkl_normal(params_true, params_pred):
    mu_true = params_true[:, :128]
    var_true = K.exp(params_true[:, 128:])
    mu_pred = params_pred[:, :128]
    var_pred = K.exp(params_pred[:, 128:])
